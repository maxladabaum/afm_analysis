...
### Windows: “Can't find a usable init.tcl”

This error means Tkinter cannot load its Tcl/Tk runtime, even if pip installed every dependency successfully. The launcher now tests actual Tk window creation before installing packages. On Windows, startup also retries with the matching Tcl/Tk libraries in the active interpreter's base installation when library discovery fails. Update the entire project, including `tk_startup.py`, and rerun the BAT file.

If the check still fails, use the python.org installer for the Python installation shown in the error. Choose **Modify** and enable **tcl/tk and IDLE**, or **Repair** if it is already enabled. Updating an older Python 3.13 installation to a newer maintenance release may also resolve runtime discovery issues. Then rerun the launcher. Installing more pip packages does not install or repair Tcl/Tk. The check can be run separately with `.venv\Scripts\python.exe tk_startup.py`.
...
Click **Run Pooled Analysis** to analyze every image in the current folder list (including discovered subfolders) and generate one persistence-length fit in a separate results window. The current minimum/maximum length, segment spacing, and threshold bias apply to every image. Each image uses its own matching `.spm` metadata or saved **Box Scale** calibration; images without either are skipped with a reason shown in the results. An animated progress window reports the current image. The app converts contours to nm before pooling point-pair measurements, so different image scales can be combined. The mean is weighted by point-pair count, not equally by image, and the fit uses those pooled means rather than averaging per-image persistence lengths. Use images representing the same polymer population and conditions.

**Export Pooled Results** saves the pooled fit and aligned Figure 2b PNGs and five CSVs under `analysis_output/polymer_persistence/pooled_<timestamp>/`: summary/settings, per-image calibration and skip status, contours, pooled MSD measurements, and aligned Figure 2b coordinates. The MSD table's `polymer_ids` refer to globally unique `pooled_id` values in the contours table, which maps them to image paths and original contour IDs; `image_ids` and `image_count` identify the contributing images at each separation. Error bars remain ±1 SD of squared point-pair distances, not a confidence interval. Pooled analysis leaves the selected image's individual results separate.

In the pooled-results window, **Preview Pooled Figure 2b** displays all accepted contours from all contributing images at a common origin with their initial tangents aligned, using the same alignment method as the individual-image Figure 2b. Coordinates are already calibrated in nm for each source image. The pooled figure contains the combined aligned-contour panel; it has no single-image AFM panel. Zoom and pan work in this view. **Show Persistence Fit** returns to the WLC plot. **Export Pooled Figure 2b** saves the aligned PNG and a CSV of aligned points with pooled IDs, source paths, and original contour IDs. The standard **Export Pooled Results** also includes these files.

Click **Contour QC** to inspect detection quality for the selected image. It shows the intensity cutoff and foreground polarity, fraction of pixels selected before cleanup, small components removed, analysis resolution, candidate count, acceptance percentage, and rejection counts for boundary contact, invalid/branched paths, and minimum/maximum length. A per-contour table lists endpoints and branch pixels. The strict tracer requires two endpoints and no pixel with more than two neighbors in its 8-neighbor graph, so even a small spur can reject an otherwise plausible polymer. Branch-pixel counts are not counts of physical junctions. Acceptance percentage refers to detected components, not detection completeness or fit confidence.

**Prune branches nm** controls short-spur removal before the branching check. The default **0** keeps strict detection. A positive value, for example **10 nm**, allows removal of terminal side branches up to that length (measured from an endpoint to its first junction). The shortest eligible spur is removed first, then the skeleton is checked again. Whole unbranched polymers and loops are not pruned; substantial branches beyond the limit remain rejected. Boundary-connected components remain excluded. Length filters and the fit use the resulting trace. Inspect the overlay as you increase the limit: pruning can remove real short features as well as image artifacts. The setting applies to individual and pooled analyses and is recorded in exports. QC reports removed branches and rescued contours, with per-contour pruning counts. Rerun analysis after changing it. The Polymers sidebar can be scrolled to reach all controls.

All polymer preview panes (original/overlay/contours, persistence fit, Figure 2b, Figure C) and the pooled-results plot support **Zoom +**, **Zoom −**, and **Fit**. Scroll over a preview to zoom, drag to pan, and double-click to fit. Each pane keeps its own zoom level. Analysis-tab plot previews also support scrolling and dragging alongside their existing zoom/reset buttons. Zoom changes only the display, not measurements or exported figures.

**Contour QC** toggles a text panel directly over the **Overlay** preview, switching to that view automatically. The panel stays readable as the preview resizes and includes a **View per-contour details** link for the full table. Click **Contour QC** again to hide the panel.

QC rejection counts assign one reason to each rejected component; boundary contact takes priority, followed by invalid topology, then length limits for valid paths. Invalid paths have blank lengths in exports, and their skeleton pixels are displayed without drawing artificial lines between unordered pixels. **Export Polymer Results** includes a QC CSV and topology fields in the contour CSV. Pooled results show per-image rejection breakdowns and include QC columns in `images.csv` and topology fields in `contours.csv`. Compare the original and overlay views when adjusting threshold bias; increasing the maximum length does not rescue a branched trace.

Use **Max length nm** to exclude contours longer than a chosen length measured along the curve. Leave it blank for no upper limit; otherwise it must be at least **Min length nm**. Click **Analyze Polymers** after changing it. Excluded long contours are marked `longer_than_max_length` in the contour CSV and do not contribute to the fit. The summary CSV records both length limits, and cached results are checked against them before export.

Contours connected to the scan boundary (including a two-pixel margin at analysis resolution) are excluded as `touches_scan_boundary`. This prevents the image frame from being mistaken for a long polymer and excludes polymers cut off at the scan edge.

Each WLC plot dot averages squared straight-line distances over all sampled point pairs at that contour separation. Polymers supplying more pairs have more weight. Error bars show ±1 sample standard deviation of those squared distances, describing their spread rather than a confidence interval for the mean or persistence length; overlapping pairs within a polymer are correlated. Dot size indicates pair count, and color indicates the number of contributing polymers. The MSD CSV includes the standard deviation, pair count (`sample_count`), `polymer_count`, and semicolon-separated `polymer_ids` matching the contour IDs. Large separations may include fewer polymers. Symmetric SD bars can extend below zero even though squared distances cannot be negative.
